import { isEmpty, trimString } from "./generalUtils.js";
...
/**
 * Extracts a human-readable title for a table row
 */
const extractRowTitle = (row, index) => {
  let rowTitle = `Row ${index + 1}`;
  const keys = Object.keys(row);
  const keyColumn = keys.find((k) => k !== "id" && k !== "addCode") || keys[0];
...
/**
 * Extracts meaningful details from a table row
 */
const extractRowDetails = (row) => {
  const details = {};
...
const parseTable = (field) => {
  ...
  const rows = validRows.map((row, index) => {
    return {
      title: extractRowTitle(row, index),
      details: extractRowDetails(row),
    };
  });
