...
// ==================
export function evaluateCondition(inputData, condition, ruleKey, localContext = {}, context = null, logPrefix = null) {
  ...
  try {
    ...
    if (fieldVal === undefined && !["is_null", "is_empty"].includes(operator)) {
      logger.warn(`${prefix} Property '${field}' was not found in either patient data or configuration row. Configuration criteria for this field will be skipped.`);
      return false;
    }
    ...
    if (!isUnary && isEmpty(inputVal)) {
      logger.warn(`${prefix} Missing comparison value for operator '${operator}'.`);
      return false;
    }
    ...
    logger.info(`${prefix} Checking if '${field}' (${fieldVal}) ${operator} '${inputVal}' ... Result: ${result}`);
    return result;

  } catch (error) {
    logger.log("error", `${prefix} Unexpected evaluation error: ${error.message}`);
