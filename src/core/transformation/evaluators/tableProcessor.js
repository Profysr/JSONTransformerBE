...
const createFieldEvaluator = (metaMap, inputData, context, sectionKey) => {
    return (fieldKey, val, localRow = {}, logPrefix = null) => {
        ...
        if (meta.canConditional) {
            const result = applyRule(inputData, val, fieldKey, localRow, context, logPrefix);

            const source = `table:${sectionKey}`;
            if (handleRuleResult(fieldKey, result, context, source, localRow, { addToContext: false, logPrefix })) {
                return result;
            }
...
export const processTableRules = (inputData, tableConfig, options = {}) => {
    ...
    const evaluateField = createFieldEvaluator(metaMap, inputData, context, sectionKey);
    const results = [];

    for (let index = 0; index < rows.length; index++) {
        const row = rows[index];
        const rowId = (primaryKeyCol && row[primaryKeyCol]) ? row[primaryKeyCol] : `Row ${index + 1}`;
        const logPrefix = `[${sectionKey}][${rowId}]`;

        // a. Evaluate parentField logic
        if (parentFieldCol) {
            const shouldAddValue = evaluateField(parentFieldCol, row[parentFieldCol], row, logPrefix);

            if (isKilled(shouldAddValue)) {
                logger.warn(`${logPrefix} Aborted: Parent field triggered a termination (KILL).`);
                return { ...shouldAddValue, sectionKey, rowIdx: index };
            }

            if (shouldAddValue == false || shouldAddValue == "false" || shouldAddValue == "") {
                logger.info(`${logPrefix} Skipped: Parent field '${parentFieldCol}' is not active.`);
        ...
        if (rowKilled) {
            logger.warn(`${logPrefix} Aborted: Field '${killResult.field}' triggered a termination (KILL).`);
