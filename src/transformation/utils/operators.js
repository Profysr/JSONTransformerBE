// ==================
// 1 Helpers & Value Resolution
// ==================
export const isNumeric = (v) =>
  v !== null && v !== "" && !isNaN(Number(v)) && isFinite(Number(v));
...
const evalNumeric = (src, tgt, op, fn) => {
  if (!isNumeric(src) || !isNumeric(tgt)) return false;
  return fn(Number(src), Number(tgt));
};

// ==================
// 2 Operators Registry
// ==================
export const OPERATORS = {
  contains: (s, t, p) => p(s).includes(p(t)),
  not_contains: (s, t, p) => !p(s).includes(p(t)),
  equals: (s, t, p) =>
    isNumeric(s) && isNumeric(t) ? Number(s) === Number(t) : p(s) === p(t),
  not_equals: (s, t, p) =>
    isNumeric(s) && isNumeric(t) ? Number(s) !== Number(t) : p(s) !== p(t),
  ...
  is_empty: (s) =>
    ...
    (typeof s === "object" && !Object.keys(s).length),
